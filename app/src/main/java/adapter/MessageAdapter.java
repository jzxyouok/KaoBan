...
import com.com.easemob.chatuidemo.AlertDialog;
import com.com.easemob.chatuidemo.Constant;
import com.com.easemob.chatuidemo.DemoHXSDKHelper;
import com.com.easemob.chatuidemo.ShowBigImage;
import com.com.easemob.chatuidemo.ShowNormalFileActivity;
import com.com.easemob.chatuidemo.ShowVideoActivity;
import com.com.easemob.chatuidemo.adapter.VoicePlayClickListener;
import com.com.easemob.chatuidemo.utils.ImageCache;
import com.com.easemob.chatuidemo.utils.ImageUtils;
import com.com.easemob.chatuidemo.utils.SmileUtils;
import com.dezhou.lsy.projectdezhoureal.BaiduMapActivity;
...
public class MessageAdapter extends BaseAdapter {
    ...
    public MessageAdapter(ChatMessage context, String username, int chatType) {
        ...
        public void handleMessage(android.os.Message message) {
            switch (message.what) {
                ...
                case HANDLER_MESSAGE_SELECT_LAST:
                    if (activity instanceof ChatMessage) {
                        ListView listView = ((ChatMessage)activity).getListView();
...
//                if(message.direct == EMMessage.Direct.SEND){
//                    Intent i1=new Intent(context, SelfActivity.class);
//                    i1.putExtra("userNum", UserNum.userNum);
//                    activity.startActivity(i1);
//                }
//                else {
//                    Intent i2=new Intent(context,SelfActivity.class);
//                    i2.putExtra("userNum",message.getFrom());
...
//        });
        ...
        if (message.direct == EMMessage.Direct.SEND) {
            ...
            statusView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (message.getType() == EMMessage.Type.TXT)
                        activity.startActivityForResult(intent, ChatMessage.REQUEST_CODE_TEXT);
                    else if (message.getType() == EMMessage.Type.VOICE)
                        activity.startActivityForResult(intent, ChatMessage.REQUEST_CODE_VOICE);
                    else if (message.getType() == EMMessage.Type.IMAGE)
                        activity.startActivityForResult(intent, ChatMessage.REQUEST_CODE_PICTURE);
                    else if (message.getType() == EMMessage.Type.LOCATION)
                        activity.startActivityForResult(intent, ChatMessage.REQUEST_CODE_LOCATION);
                    else if (message.getType() == EMMessage.Type.FILE)
                        activity.startActivityForResult(intent, ChatMessage.REQUEST_CODE_FILE);
                    else if (message.getType() == EMMessage.Type.VIDEO)
                        activity.startActivityForResult(intent, ChatMessage.REQUEST_CODE_VIDEO);
            ...
            if(!((ChatMessage)activity).isRobot && chatType != EMMessage.ChatType.ChatRoom){
                // 长按头像，移入黑名单
                holder.iv_avatar.setOnLongClickListener(new View.OnLongClickListener() {
                    ...
                    public boolean onLongClick(View v) {
                        ...
                        intent.putExtra("cancel", true);
                        intent.putExtra("position", position);
                        activity.startActivityForResult(intent, ChatMessage.REQUEST_CODE_ADD_TO_BLACKLIST);
                        return true;
                    }
                });
            }
    ...
    private void setUserAvatar(final EMMessage message, final ImageView imageView){
        ...
        if(message.direct == EMMessage.Direct.SEND){
            String url = URLSet.serviceUrl+"/kaoban/userInformation/?userNum="+ UserNum.userNum;
...
//                            }
            ...
            String url = URLSet.serviceUrl+"/kaoban/userInformation/?userNum="+message.getFrom();
            AsyncHttpClient client = new AsyncHttpClient();
            client.get(url, new AsyncHttpResponseHandler() {
                @Override
                public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
            ...
            public boolean onLongClick(View v) {
                activity.startActivityForResult(
                        (new Intent(activity, ContextMenu.class)).putExtra("position", position).putExtra("type",
                                EMMessage.Type.TXT.ordinal()), ChatMessage.REQUEST_CODE_CONTEXT_MENU);
                return true;
            }
        });
    ...
    private void handleImageMessage(final EMMessage message, final ViewHolder holder, final int position, View convertView) {
        ...
        holder.iv.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                activity.startActivityForResult(
                        (new Intent(activity, ContextMenu.class)).putExtra("position", position).putExtra("type",
                                EMMessage.Type.IMAGE.ordinal()), ChatMessage.REQUEST_CODE_CONTEXT_MENU);
                return true;
            }
        });
    ...
    private void handleVideoMessage(final EMMessage message, final ViewHolder holder, final int position, View convertView) {
        ...
        holder.iv.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                activity.startActivityForResult(
                        new Intent(activity, ContextMenu.class).putExtra("position", position).putExtra("type",
                                EMMessage.Type.VIDEO.ordinal()), ChatMessage.REQUEST_CODE_CONTEXT_MENU);
                return true;
            }
        });
    ...
    private void handleVoiceMessage(final EMMessage message, final ViewHolder holder, final int position, View convertView) {
        ...
        holder.iv.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                activity.startActivityForResult(
                        (new Intent(activity, ContextMenu.class)).putExtra("position", position).putExtra("type",
                                EMMessage.Type.VOICE.ordinal()), ChatMessage.REQUEST_CODE_CONTEXT_MENU);
                return true;
            }
        });
        if (((ChatMessage)activity).playMsgId != null
                && ((ChatMessage)activity).playMsgId.equals(message
        ...
        holder.ll_container.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (file != null && file.exists()) {
                    // 文件存在，直接打开
                    com.easemob.util.FileUtils.openFile(file, (Activity) context);
                } else {
                    // 下载
                    context.startActivity(new Intent(context, ShowNormalFileActivity.class).putExtra("msgbody", fileMessageBody));
                }
                if (message.direct == EMMessage.Direct.RECEIVE && !message.isAcked && message.getChatType() != EMMessage.ChatType.GroupChat && message.getChatType() != EMMessage.ChatType.ChatRoom) {
                    try {
                        EMChatManager.getInstance().ackMessageRead(message.getFrom(), message.getMsgId());
                        message.isAcked = true;
    ...
    private void handleLocationMessage(final EMMessage message, final ViewHolder holder, final int position, View convertView) {
        ...
        locationView.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                activity.startActivityForResult(
                        (new Intent(activity, ContextMenu.class)).putExtra("position", position).putExtra("type",
                                EMMessage.Type.LOCATION.ordinal()), ChatMessage.REQUEST_CODE_CONTEXT_MENU);
                return false;
            }
        });
        ...
        if (imageCache.containsKey(imageURL)) {
            ...
            if (null != cacheFiles) {
                ...
                if (i < cacheFiles.length) {
                    return BitmapFactory.decodeFile(Environment.getExternalStorageDirectory() + "/kaoban/" + bitmapName, option);
                }
            }
        }
        return null;
